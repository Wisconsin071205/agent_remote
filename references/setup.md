...
```powershell
./scripts/install-vlab.ps1 -IdentityFile "$HOME/.ssh/vlab-identity.pem"
```
...
```powershell
./scripts/vasp-agent.ps1 connect -IdentityFile "$HOME/.ssh/vlab-identity.pem"
```
...
```powershell
./scripts/vasp-agent.ps1 status -IdentityFile "$HOME/.ssh/vlab-identity.pem"
./scripts/vasp-agent.ps1 whoami -IdentityFile "$HOME/.ssh/vlab-identity.pem"
./scripts/vasp-agent.ps1 jobs -IdentityFile "$HOME/.ssh/vlab-identity.pem"
```

Set `VLAB_IDENTITY_FILE` to avoid repeating the key path:

```powershell
$env:VLAB_IDENTITY_FILE = "$HOME/.ssh/vlab-identity.pem"
```
...
- An old flat file such as `{"remote_root": "/public/home/user"}` is migrated automatically to `servers.cl9` (merged over the defaults) and written back atomically on first run.
